...
function App() {
  ...
  const handleDeposit = async () => {
    setShowWidget(true);
    try {
      const response = await fetch('https://api-dev.layerswap.cloud/api/v2-alpha/limits?source_network=ARBITRUM_MAINNET&source_token=USDC.e&destination_network=ARBITRUM_MAINNET&destination_token=ETH&deposit_mode=wallet&refuel=false', {
        method: 'GET',
        headers: {
